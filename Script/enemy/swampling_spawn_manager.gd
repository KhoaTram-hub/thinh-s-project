...
@export var swampling_scene: PackedScene 
...
# Biến tĩnh để dứt điểm lỗi nhân bản (90/30)
...
func spawn_swampling() -> void:
	var pos = get_valid_spawn_position()
	var swampling = swampling_scene.instantiate()
	swampling.position = pos
	
	swampling.add_to_group("enemies") 
	get_parent().add_child.call_deferred(swampling)
